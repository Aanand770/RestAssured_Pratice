...
public class Basics {
	...
	public void BookApiCrud(String isbn, String aisle, String author, String name) {
		...
		String AddBookResponse = given().log().all().header("Content-Type", "application/json")
				.body(Payload.addBook(isbn, aisle, author, name)).when().post("Library/Addbook.php").then().log().all()
		...
		String GetBookResponseAfterDelete = given().log().all().header("Content-Type", "application/json")
				.queryParam("ID", BookId).when().get("/Library/GetBook.php").then().log().all().statusCode(404)
	...
	public Object[][] getData() {
		...
		return new Object[][] { { "acd", "123", "Anand Bohara", "Postman Automation with JavaScript" },
				{ "bcd", "124", "Rahul", "Automation BDD Framework From Scratch" },
				{ "ccd", "125", "Jhon", "Java Basics" } };
	}
}
